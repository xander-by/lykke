...
const buySell = async (pair, buySell) => {
  ...
  // ПОКА БЕЗ ПРОВЕРКИ ОБЪЕМА
  if (
    wallet[val2] > settings.minMyVolSell &&
...
const makeOrder = async (pair, resOrdersArray, typeBuySell, settings) => {
  ...
  const val =
    typeBuySell === "Sell" ? pair.slice(0, 3) : pair.slice(-settings.sufLen); // USD - BTCUSD

  let suffix = 0;
  if (typeBuySell === "Sell")
    suffix = settings.volCorrectionSell
      .toFixed(settings.volDepth)
      .slice(-settings.sufLen);
  else
    suffix = settings.volCorrectionBuy
      .toFixed(settings.volDepth)
      .slice(-settings.sufLen);
  ...
  for (let i = 0; i < 5; i++) {
    console.log(
      "......  " +
        resOrdersArray[index].Prices[i].Price +
        "  " +
        resOrdersArray[index].Prices[i].Volume
    );
  }
  ...
  for (const el of resOrdersArray[index].Prices) {
    ...
    // IF IT IS OUR ORDER
    if (
      plusMinus * el.Volume.round(settings.volDepth - 4) ===
        lastOrderVol.round(settings.volDepth - 4) &&
      el.Price.round(settings.priceDepth) ===
        lastOrderPrice.round(settings.priceDepth)
    ) {
      isOur = true;
  ...
  // firstValue.Price
  const myPrice = (
    firstValue.Price.round(settings.priceDepth) +
    plusMinus * settings.priceAdd
  ).round(settings.priceDepth);

  // set new order
  if (
    new Date().getSeconds() === 7 ||
    !ourPriceInOrder ||
    !lastOrderPrice ||
    (lastOrderPrice &&
      (typeBuySell === "Buy"
        ...
        : firstValue.Price.round(settings.priceDepth) <
          lastOrderPrice.round(settings.priceDepth))) ||
    (lastOrderPrice &&
      plusMinus * (lastOrderPrice - firstValue.Price) > settings.priceAdd * 50)
  ) {
    ...
    const myVolume = (
      (
        wallet[val].round(settings.volDepth) /
        (typeBuySell === "Sell"
          ? 1
          : firstValue.Price.round(settings.priceDepth))
      ).floor(settings.floorVolume) - newVolCorrection
    ...
    console.log(
      "++firstValue price " + firstValue.Price.round(settings.priceDepth)
    );
    console.log(
      "++firstValue Volume " +
        plusMinus * firstValue.Volume.round(settings.volDepth)
    );
    console.log("--lastOrderVol " + lastOrderVol.round(settings.volDepth));
    console.log(
      "--lastOrderPrice " + lastOrderPrice.round(settings.priceDepth)
    );
    ...
    if (
      ...
      console.log(
        "order: 50...   " +
          (plusMinus * (lastOrderPrice - firstValue.Price) >
            settings.priceAdd * 50)
      );
      console.log(
        `${plusMinus * (lastOrderPrice - firstValue.Price)} > ${
          settings.priceAdd * 50
    ...
    if (response.Id) {
      ...
      typeBuySell === "Buy"
        ? (settings.lastOrderBuyPrice = myPrice)
        : (settings.lastOrderSellPrice = myPrice);
      typeBuySell === "Buy"
        ? (settings.lastOrderBuyVol = myVolume)
        : (settings.lastOrderSellVol = myVolume);
    } else {
      typeBuySell === "Buy"
        ? (settings.lastOrderBuyPrice = 0)
        : (settings.lastOrderSellPrice = 0);
      typeBuySell === "Buy"
        ? (settings.lastOrderBuyVol = 0)
        : (settings.lastOrderSellVol = 0);
    }
